...
/**
 * Official social brand marks (simple-icons vector data, CC0) rendered as
 * currentColor glyphs. Profile URLs are owner-supplied (2026-07-04) and
 * live in siteConfig.socials.
 */
    ...
    <ul className={className ?? "flex items-center gap-1"}>
      {Object.entries(siteConfig.socials).map(([key, href]) => (
        <li key={key}>
          <a
            href={href}
            // Full legal name in the accessible name, because this string is
            // what a screen reader announces and what an answer engine reads
            // as the relationship between the company and the profile.
            aria-label={`Visit ${siteConfig.legalName} on ${LABELS[key]}`}
            className={
              key === "nextdoor"
                ? "flex h-8 items-center justify-center rounded-full px-1.5 text-steel-300 transition-colors hover:bg-white/10 hover:text-white"
                : "flex size-8 items-center justify-center rounded-full text-steel-300 transition-colors hover:bg-white/10 hover:text-white"
            }
          >
            {key === "nextdoor" ? (
