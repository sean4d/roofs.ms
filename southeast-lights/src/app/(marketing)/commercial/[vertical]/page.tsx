...
import { VERTICALS, verticalBySlug, verticalsByPriority } from "@/config/verticals";
...
export async function generateMetadata({
  ...
  if (!vertical) return {};
  return pageMetadata({
    title: `${vertical.title} | Southeast Lights`,
    ...
    <>
      ...
      >
        <div className="grid gap-4 md:grid-cols-2">
          {vertical.concerns.map((concern) => (
            <div key={concern.heading} className="card-lit flex flex-col gap-3 p-7">
              <h3 className="text-lg font-semibold">{concern.heading}</h3>
              <p className="leading-relaxed text-bone-400">{concern.body}</p>
            </div>
          ))}
        </div>
      </Section>
      ...
      >
        ...
        <p className="mt-6 max-w-2xl text-sm leading-relaxed text-bone-500">
          For projects of real scale we travel statewide. If a project is worth
          doing, distance is a scheduling question rather than a reason to say
          no.
        </p>
      </Section>

      <Section eyebrow="Questions" title="What organisations ask us">
        <FaqList items={faqs} />
      </Section>

      <Section tone="raised" eyebrow="Also for" title="Other properties we light">
